...
import { GroupsListScreen,GroupSearchScreen } from '../ScreenNames/ScreenNames';
...
export default class GroupsScreen extends Component<{}> {
    ...
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            title: (user.lang == 'ar')?'المجموعات':'Groups',
    ...
    async init (){
        const deals = await ajax.getMainGroups(user.lang);
        for (var i=0;i<deals.Statuses.length;i++)
        {
          deals.Statuses[i].key = deals.Statuses[i].Id+"";
          deals.Statuses[i].ItemType = "Group";
        }
        this.setState({ deals });
    }
    setCurrentDeal = (dealId) => {
        user.selectedGroup = dealId;
        this.props.navigation.navigate(GroupsListScreen);
    };
    render() {
