...
export default class AllInboxDetailsScreen extends Component<{}> {
    ...
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            tabBarLabel:(user.lang == 'ar')?'المقروء':'Read',
        }
      }
    ...
    async componentDidMount() {
        const deals = await ajax.fetchInboxDetails(user.InboxType,1,1,(user.lang == 'ar'));
        ...
        return (
            <View style={styles.container}>
                {(this.state.deals.Details && this.state.deals.Details.length > 0) ?
                    (<InboxList deals={this.state.deals.Details} inboxType={1}  type="List" onItemPress={this.setCurrentDeal}/>)
                    : (<View>
