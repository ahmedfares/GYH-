...
import { VoucherListScreen, VoucherSearchScreen } from '../ScreenNames/ScreenNames';
...
export default class VoucherScreen extends Component<{}> {
    ...
    static navigationOptions = ({ navigation }) => {
        ...
        return {
            title: (user.lang == 'ar')?'الفاوتشرات':'Vouchers',
      ...
      searchContents() {
        this.props.navigation.navigate(VoucherSearchScreen);
      }
    ...
    async init (){
        const deals = await ajax.getMainVoucher(user.lang);
        for (var i=0;i<deals.Statuses.length;i++)
        {
          deals.Statuses[i].key = deals.Statuses[i].Id+"";
          deals.Statuses[i].ItemType = "Voucher";
        }
        this.setState({ deals });
    }
    setCurrentDeal = (dealId) => {
        user.selectedVoucher = dealId;
